...
from dbtemplates.models import (
...
# Check if django-reversion is installed and use reversions
# VersionAdmin as the base admin.
...
class CodeMirrorTextArea(forms.Textarea):
    """
    Text area using CodeMirror Widget.
...
class TemplateAdmin(TemplateModelAdmin):
    ...
    def repopulate_cache(self, request, queryset):
        """Action to repopulate cache with selected templates."""
        for template in queryset:
    ...
    repopulate_cache.short_description = _(
        "Repopulate cache with " "selected templates")

    def check_syntax(self, request, queryset):
        """Action to check the syntax of template."""
        errors = []
        for template in queryset:
